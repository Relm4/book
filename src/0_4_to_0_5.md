...
In this chapter, all major changes should be covered to help you to migrate your existing code quickly.
...
Components are now more similar to `MicroComponent` which means they are much more flexible and don't need a strong connection to their parent component.
...
3. Regarding updates, there hasn't been a lot of changes. However you can now update view and model at once using [`update_with_view`](https://relm4.org/docs/next/relm4/trait.Component.html#method.update_with_view).
4. The [shutdown](https://relm4.org/docs/next/relm4/trait.Component.html#method.shutdown) method is called when the component is destroyed.
...
Not you have a [`Connector`](https://relm4.org/docs/next/relm4/component/struct.Connector.html).
...
For types that implement `Componet` that don't have any widgets (such as implementers of the `Worker` trait), you can TODO!
...
`Worker` is a even simpler variant of `SimpleComponent` that helps with the implementation of `Component`.
Particularly, it doesn't support widgets and allows running the components update loop on a different thread by using TODO!.
...
Messages can now be passed by using the [`ouput_to_parent_msg`](https://relm4.org/docs/next/relm4/factory/traits/trait.FactoryComponent.html#method.output_to_parent_msg) method.
It allow to optionally forward messages to the parent.
...
+ Additional arguments are now passed with brackets `[]` instead of parenthesis `()`.
...
Now you need to specify an application id and the generic parameter is only passed when calling `run()`, not when calling `new()`.
